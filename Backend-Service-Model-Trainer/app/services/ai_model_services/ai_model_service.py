...
async def create_model(session: AsyncSession, model: AIModel) -> AIModel:
...
async def update_model(session: AsyncSession, model_id: int, updated_model: AIModel) -> AIModel:
    db_model = await ai_model_repo.get_ai_model(session, model_id)
    if not db_model:
        raise HTTPException(status_code=404, detail="Model not found")
    return await ai_model_repo.update_ai_model(session, db_model, updated_model)
